...
class SoulCoin {
    ...
    lerDados() {
        ...
        var cotacaoVenda = ((cotacaoCompra) - ((cotacao2 * (cotacaoCompra * 0.1)) + (cotacaoCompra * 0.08)));
        ...
        for (let m = 0; m < this.arrayMoeda.length; m++) {
            if (id == this.arrayMoeda[m].id) {
                ...
                if (this.arrayMoeda[m].operacao == "Compra") {
                    this.arrayMoeda[m].valor = (Math.sqrt(Math.sqrt(Math.pow(this.arrayMoeda[m].quantidade, 3))) + (this.arrayMoeda[m].quantidade * this.arrayMoeda[m].cotacao)).toFixed(4)
                }
                else if (this.arrayMoeda[m].operacao == "Venda") {
                    this.arrayMoeda[m].valor = (((Math.sqrt(Math.sqrt(Math.pow(this.arrayMoeda[m].quantidade, 3))) + (this.arrayMoeda[m].quantidade * this.arrayMoeda[m].cotacao))) - ((this.arrayMoeda[m].cotacao2 * ((Math.sqrt(Math.sqrt(Math.pow(this.arrayMoeda[m].quantidade, 3))) + (this.arrayMoeda[m].quantidade * this.arrayMoeda[m].cotacao)) * 0.1)) + ((Math.sqrt(Math.sqrt(Math.pow(this.arrayMoeda[m].quantidade, 3))) + (this.arrayMoeda[m].quantidade * this.arrayMoeda[m].cotacao)) * 0.08))).toFixed(4)
                }
            }
        }
        $("#tipoOperacao").attr("disabled", false);
